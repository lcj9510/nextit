	...
	<!-- START : 페이지네이션  -->
		<nav class="text-center">
			<ul class="pagination">
				...
				<!-- 이전 페이지 -->
				<!-- 1~10일때는 요게 false 그외는 true -->
				<c:if test="${searchVO.firstPage != 1 }">
					<li><a href="memberList.wow?curPage=${searchVO.firstPage -1 }" data-page="${searchVO.firstPage -1 }"><span
							aria-hidden="true">&lt;</span></a></li>
				</c:if>
				<!-- 페이지 넘버링  -->
				<!-- forEach를 쓰는데 firstPage, lastPage = begin, end -->
...
<script type="text/javascript">
		...
		$('ul.pagination li a[data-page]').click(function(e) {
			//a링크를 누르면 get방식으로 감 -> 파라미터가 curPage 1개만 넘어감
			//searchVO에 해당하는 데이터가(search *, rowSizePerPage 등 + curPage) == form 태그 안에 input들
			//데이터가 같이 넘어가도록 get방식 대신에 form태그 post submit하도록 변경
			//검색도 했고 현재 5번글이다. --> 8번글을 눌렀다.
			//검색, rowSizePerPage이 유지되면서 curPage만 8로 바뀌면 됩니다.
			
			//기본이벤트 막기, curPage변경(햔재 내가 누른 a태그의 data-page에 있음), form submit
			
			//a링크 기본은 파라미터로 curPage만 넘어갑니다.
			//검색조건이랑 함께 curPage도 같이 넘어가도록 a링크 기본이벤트 막고
			//form태그의 submit메소드를 실행합시다.
			
			//a링크 기본이벤트 preventDefault
			//form태그 밑의 input type hidden name=curPage인 태그의 value값을
			//현재 a링크 data-page의 값으로 변경
			//form태그 submit
			
			//data속성 : data를 담기 위한 속성
			//<a href="#" data-aaaa="삼십" data-bbb="25" data-ccc="0" id="myA"></a>
			/* $("#myA").data("aaaa"); == 삼십
			$("#myA").data("bbb"); == 25
			$("#myA").data("ccc", 5); data-ccc=5 */
			e.preventDefault();
